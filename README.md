...
[![OpenEnv](https://img.shields.io/badge/OpenEnv-spec-green.svg)](https://github.com/openenv/openenv)
...
software company on earth — and one of the worst things to do at 3 AM.
Public industry numbers:

| Metric | Value |
|---|---|
| Average annual cost of incident management per company | **$1.5M** |
| Mean time to resolve a production incident | **69 minutes** |
| Companies running 24/7 on-call rotations | **>90%** of mid+ tech orgs |
...
The hard task buries a third root cause (`session_store`, a slow memory
leak) at medium severity among 14 alerts. It is only discoverable **after**
fixing the first two — pure chained-operations reasoning, the kind of
multi-step inference that distinguishes weak agents from frontier ones.

---

## Score variance proves difficulty progression

Same env, same prompt, same Docker image — only the model changes:

| Model | Easy | Medium | Hard | Notes |
|---|---|---|---|---|
| **Frontier model (e.g. GPT-5)** | **1.000** | **1.000** | **1.000** | Solves hard in ~13 steps, finds all 3 root causes |
| **Mid-tier model (e.g. GPT-4o-mini)** | 1.000 | 0.857 | **0.538** | Loops on hard, never discovers `session_store` |
| **Open 70B-class model (e.g. Qwen 2.5)** | 1.000 | 0.571 | ~0.40 | Misses critical-alert acknowledgement rubric |
| Random / no-action | 0.143 | 0.143 | 0.077 | Vacuous "no incorrect fixes" rubric only |

**Hard task is genuinely hard for non-frontier models.** Mid-tier agents fix
the two obvious root causes, then enter a loop because 5 services are still
degraded but they can't figure out why. Frontier models reason through the
dependency graph and find the third root cause.
...
multiple models with distinct scores.
...
# Pull and run from the published HF Space
...
1. **Real domain, not a toy.** SRE incident triage is something every tech
   ...
   the actual on-call workflow.

2. **Genuinely challenges frontier models.** Mid-tier models score around 0.54
   on the hard task while frontier models score 1.000. That's the kind of
   variance that makes a benchmark useful.

3. **Eight hardening techniques** applied across the three tasks: exploratory
   discovery, indirect references, chained operations, bulk-conditional treatment,
   cross-feature actions, disambiguation, non-obvious paths, and state-dependent
   logic. The hard task uses all eight.

4. **Binary rubric grading.** Each criterion is 0 or 1, final score is the
   average — deterministic, reproducible, and impossible to game with
   fractional rewards.

5. **75 unit tests + clean-room validator pass.** Every grader is proven
   correct on initial state (all 0) and on the golden path (all 1).

6. **Lightweight infra.** Docker container uses ~130 MiB of memory and ~6%
   of one CPU on the 2 vCPU / 8 GB target machine. Inference for all 3 tasks
   completes in under a minute with a strong model.
...
├── scenarios.py           # 3 task definitions: services, alerts, dependencies
├── client.py              # WebSocket client (judges connect through this)
├── server/
│   ├── app.py             # FastAPI wiring via create_app()
│   ├── environment.py     # Core simulation: cascading recovery, binary rubrics
│   └── web_ui.py          # Custom Gradio playground for /web
├── tests/                 # 75 tests: grader sanity, edge cases, scenario integrity
├── inference.py           # Spec-compliant LLM agent (from_docker_image / from_env)
├── openenv.yaml           # Environment manifest
├── Dockerfile             # Port 8000, ENABLE_WEB_INTERFACE=true
├── pyproject.toml         # [project.scripts] server entry
